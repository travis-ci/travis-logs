# frozen_string_literal: true

source 'https://rubygems.org'

ruby '3.2.2'

git_source(:github) { |repo| "https://github.com/#{repo}.git" }

gem 'activesupport', '>= 7.0.6'
gem 'aws-sdk'
gem 'bunny'
gem 'coder', github: 'piotrmilcarz/coder'
gem 'concurrent-ruby', require: 'concurrent'
gem 'connection_pool'
gem 'dalli'
gem 'jemalloc', github: 'piotrmilcarz/jemalloc-rb'
gem 'jwt'
gem 'libhoney'
gem 'metriks', github: 'travis-ci/metriks'
gem 'metriks-librato_metrics', github: 'travis-ci/metriks-librato_metrics'
gem 'multi_json'
gem 'opencensus'
gem 'opencensus-stackdriver'
gem 'pg'
gem 'pry'
gem 'puma'
gem 'pusher'
gem 'rack-ssl'
gem 'rack-test', group: :test
gem 'rake'
gem 'rbtrace'
gem 'redis-namespace', '>= 1.11.0'
gem 'redlock'
gem 'rspec', group: :test
gem 'rubocop', require: false, group: :test
gem 'sentry-raven'
gem 'sequel'
gem 'sidekiq', '~> 7.1.2'
gem 'simplecov', require: false, group: :test
gem 'sinatra', '>= 3.0.6'
gem 'sinatra-contrib'
gem 'sinatra-param'
gem 'stackprof'
gem 'travis-config', github: 'travis-ci/travis-config', branch: 'prd-ruby-upgrade-dev'
gem 'travis-exceptions', github: 'travis-ci/travis-exceptions', branch: 'prd-ruby-upgrade-dev'
gem 'travis-lock', github: 'travis-ci/travis-lock', branch: 'prd-ruby-upgrade-dev'
gem 'travis-logger', github: 'travis-ci/travis-logger', branch: 'prd-ruby-upgrade-dev'
gem 'travis-metrics', github: 'travis-ci/travis-metrics', branch: 'prd-ruby-upgrade-dev'
